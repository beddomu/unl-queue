...
class LinkAccount(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            account = None
            async with session.get("https://ddragon.leagueoflegends.com/api/versions.json") as version_json:
                version = await version_json.json()
            try:
                async with session.get("https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-name/{}?api_key={}".format(account_name, os.getenv("RIOT_API_KEY"))) as champ_json:
                    account = await champ_json.json()
            ...
            if account:
                ...
                if view.value is None:
                    ...
                    if not str(interaction.user.id) in unlq_json['players'].keys():
                        ...
                        unlq_json['players'][str(interaction.user.id)]['points'] = 200
                        unlq_json['players'][str(interaction.user.id)]['unp'] = 0
                    ...
                    unlq_json['players'][str(interaction.user.id)]['accountId'] = account['accountId']
                    unlq_json['players'][str(interaction.user.id)]['summonerIconId'] = account['profileIconId']
                    await friend_request(account['name'])
